    ...
    <div class="organizationalDetail">
      <div class="setDetail">
        <div class="organizationalDetailTitle">
          <span class="name">{{detailObj.orgName || detailObj.departmentName}}</span>
          <el-button plain size="small" icon="el-icon-plus" v-if="detailObj.orgAttribute === '01'" @click="addRegion">添加区域</el-button>
          <el-button plain size="small" icon="el-icon-plus" v-if="detailObj.orgAttribute !== '04'" @click="addNextDepartments">添加下级部门</el-button>
          <el-button plain size="small" icon="el-icon-edit-outline" v-if="detailObj.orgAttribute === '04'">修改</el-button>
          <el-button plain size="small" icon="el-icon-delete" v-if="detailObj.orgAttribute === '04'">删除</el-button>
        </div>
        <p class="employeeNumber" v-if="detailObj.orgAttribute !== '04'">
          {{detailObj.orgAttribute === '01' ? '系统管理员' : '公司管理员' }} 
          <span>{{detailObj.managerName}}/{{detailObj.managerCode}}</span>
          <el-button plain size="small" icon="el-icon-edit-outline"
            v-if="detailObj.orgAttribute !== '01'" style="margin-left:10px;">
            修改
          </el-button>
        </p>
        ...
        if (node.level === 0) {
          ...
          getDepartmentList(node.data.id).then(res=>{
            if(res.results.length>0){
              let val = Array.from(node.data.children);
              let newVal = res.results.concat(val);
              newVal.forEach(item=>{
      ...
      _getTreeOrgs() {
        getTreeOrgs().then(res => {
          ...
          if(res.results && res.results.length){
            // 获取总公司列表
            let headquartersList = res.results.filter(item=>{
              return item.orgAttribute === '01';
            })
            // 获取不是总公司的列表
            let headquartersOtherList = res.results.filter(item=>{
              return item.orgAttribute !== '01';
            })
            headquartersOtherList.forEach(item => {
              item.children = [];
            });
            headquartersList[0].children = headquartersOtherList;
            this.treeOrgsList = headquartersList;
            this.detailObj = JSON.parse(JSON.stringify(headquartersList[0]));
