        ...
        >
          ...
          <el-table-column prop="quantityIn" label="入库数量" show-overflow-tooltip>
            <template slot-scope="scope">
              ...
              >
                <el-input
                  @change="changeQuantityIn(scope.row)"
                  placeholder="请填写入库数量"
          ...
          <el-table-column prop="averagePrice" label="单价(元)" width="100">
            <template slot-scope="scope">
              ...
              >
                <el-input
                  @change="changeAveragePrice(scope.row)"
                  placeholder="请填写单价(元)"
          ...
          <el-table-column prop="useYear" label="使用年限(年)" width="120" show-overflow-tooltip>
            <template slot-scope="scope">
              ...
              >
                <el-input
                  @change="changeUseYear(scope.row)"
                  placeholder="请填写使用年限(年)"
                  v-model="scope.row.useYear"
                ></el-input>
              </el-form-item>
            </template>
          </el-table-column>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      model: {
        rules: {
          classifyName: [{ required: true, message: "", trigger: "blur" }],
          materialName: [{ required: true, message: "", trigger: "blur" }],
          ...
          averagePrice: [
            { validator: ModalcheckAveragePrice, trigger: "blur" }
          ],
          useYear: [
            {
  ...
  methods: {
    ...
    handleSuccess(response, file, fileList) {
      if (response.status !== 0) {
        this.$message.error(
          "该文件不符合Excel导入标准，请获取Excel导入模板导入"
        );
        return;
      }
      ...
      this.model.newfilepathList = JSON.parse(
        JSON.stringify(this.newfilepathList)
      );
