<template>
  <div class="orgIncoming">
    ...
    <el-dialog center title="固定资产入库审批流程提交" :visible.sync="dialogProcessWindowdetailVisible">
      <process-window-detail :sid="sid" :projectId="projectId" :processObj="submitinf" @isCancle="isCancle"
                             @isSubmit="isSubmit"></process-window-detail>
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      handleExport() {
        ...
        exportexcelIncoming(data).then(data => {
          if (!data) {
            return
          }
