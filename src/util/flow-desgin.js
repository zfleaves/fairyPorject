...
import {
  moveProcessNode, createProcessNode,
  deleteProcessNode, deleteProcessNodeLine,
  checkProcessesConnect
} from "../api/processDefinition/process";
  ...
  function init(divId) {
    ...
    _designer.addModelChangedListener(function (evt) {
      ...
      txn.changes.each(function (e) {
        ...
        if (e.change === go.ChangedEvent.Insert) { //添加
          // console.log(evt.propertyName + " added node with key: " + e.newValue.key);
          console.log(e.newValue);
          const data = {
            processCode,
            nodeCode: e.newValue.nodeCode,
            indexX: e.newValue.loc.split(' ')[0],
            indexY: e.newValue.loc.split(' ')[1],
          }
          createProcessNode(data).then(res => {
            if (res.status === 0) {
              let results = res.results;
              let newNode = e.newValue;
              newNode.processNodeCode = results.processNodeCode;
              newNode.processCode = results.processCode;
              newNode.canRemove = 1;  // 允许删除
              // _designer.startTransaction("vacate");
              _designer.model.setDataProperty(newNode, "key", results.processNodeCode);
        ...
        } else if (e.change === go.ChangedEvent.Remove) { // 删除
    ...
    _designer.addDiagramListener("LinkDrawn", function (e) {
      ...
      let node = e.subject.data;
      if (!node.to || !node.from) {
        e.diagram.commandHandler.deleteSelection(); // 删除连线
        return
      }
      checkProcessesConnect(node.from, node.to, processCode).then(res => {
        if (res.status === 0) {
         if(res.results.status===1){
           e.diagram.commandHandler.deleteSelection(); // 删除连线
           Message.error(res.results.errorMsg);
         }
        }else {
          Message.error(res.errorMessage);
        }
    ...
    _designer.addDiagramListener("SelectionMoved", function (e) {
      ...
      e.subject.each(function (n) {
        //n为删除节点或线的对象
        console.log(n.data);
        let x = n.data.loc.split(' ')[0];
        let y = n.data.loc.split(' ')[1];
        moveProcessNode(n.data.key, x, y).then(res => {
          if (res.status !== 0) {
            Message.error(res.errorMessage);
          }
        })
    ...
    _designer.addDiagramListener("SelectionDeleting", function (e) {
      ...
      e.subject.each(function (n) {
        ...
        if (n.data.nodeCode === 'BEGIN' || n.data.nodeCode === 'END') {
          Message.error('开始和结束步骤不可删除');
          e.cancel = true;
          return
        }
        if (n.data.key) {
          deleteProcessNode(n.data.processNodeCode).then(res => {
            if (res.status !== 0) {
              Message.error(res.errorMessage);
            }
          })
        ...
        if (n.data.from && n.data.to) {
          deleteProcessNodeLine(n.data.from, n.data.to).then(res => {
            if (res.status !== 0) {
              Message.error(res.errorMessage);
            }
          })
